...
public:
  ...
  ALWAYS_INLINE bool IsValid() const { return m_memory.size > 0; }
  ALWAYS_INLINE bool IsMapped() const { return true; }
  ALWAYS_INLINE const char* GetMappedPointer() const
  {
    return g_deko3d_context->GetGeneralHeap().CpuAddr<char>(m_memory);
  }
  ALWAYS_INLINE char* GetMappedPointer() { return g_deko3d_context->GetGeneralHeap().CpuAddr<char>(m_memory); }
  ALWAYS_INLINE u32 GetMappedStride() const { return m_map_stride; }
  ALWAYS_INLINE u32 GetWidth() const { return m_width; }
  ALWAYS_INLINE u32 GetHeight() const { return m_height; }

  ALWAYS_INLINE DkGpuAddr GetGPUAddr() const { return g_deko3d_context->GetGeneralHeap().GpuAddr(m_memory); }

  bool Create(DkImageFormat format, u32 width, u32 height);
  ...
  void CopyFromTexture(dk::CmdBuf command_buffer, Texture& src_texture, u32 src_x, u32 src_y, u32 src_layer,
  ...
  void CopyToTexture(dk::CmdBuf command_buffer, u32 src_x, u32 src_y, Texture& dst_texture, u32 dst_x, u32 dst_y,
