...
#include "context.h"
#include "util.h"
Log_SetChannel(Deko3D::StreamBuffer);
...
StreamBuffer::StreamBuffer(StreamBuffer&& move)
  : m_size(move.m_size), m_current_offset(move.m_current_offset), m_current_space(move.m_current_space),
    m_current_gpu_position(move.m_current_gpu_position), m_buffer(move.m_buffer), m_host_pointer(move.m_host_pointer),
...
{
  MemoryHeap::Allocation new_buffer = g_deko3d_context->GetGeneralHeap().Alloc(size, 256);
...
void StreamBuffer::Destroy(bool defer)
{
  ...
  {
    if (defer)
      g_deko3d_context->DeferedFree(&g_deko3d_context->GetGeneralHeap(), m_buffer);
    else
      g_deko3d_context->GetGeneralHeap().Free(m_buffer);
  }
...
void StreamBuffer::UpdateCurrentFencePosition()
{
  ...
  const u64 counter = g_deko3d_context->GetCurrentFenceCounter();
...
void StreamBuffer::UpdateGPUPosition()
{
  ...
  const u64 completed_counter = g_deko3d_context->GetCompletedFenceCounter();
...
bool StreamBuffer::WaitForClearSpace(u32 num_bytes)
{
  ...
  if (iter == m_tracked_fences.end() || iter->first == g_deko3d_context->GetCurrentFenceCounter())
    return false;

  // Wait until this fence is signaled. This will fire the callback, updating the GPU position.
  g_deko3d_context->WaitForFenceCounter(iter->first);
